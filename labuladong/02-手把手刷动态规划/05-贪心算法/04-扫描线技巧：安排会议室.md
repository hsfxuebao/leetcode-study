...
当然，这里背包的约束不是一个最大重量，而是各个物品（会议）不能互相冲突。把各个会议按照结束时间进行排序，然后参考前文 [0-1 背包问题详解](https://labuladong.github.io/algo/di-er-zhan-a01c6/bei-bao-le-34bd4/jing-dian--8780f/) 的思路即可解决，等我以后有机会可以写一写这个问题。
...
老读者肯定可以联想到之前说过的一个算法技巧： [差分数组技巧](https://labuladong.github.io/algo/di-yi-zhan-da78c/shou-ba-sh-48c1d/xiao-er-me-c31c8/)。
...
不过，这个解法的效率不算高，所以我这里不准备具体写差分数组的解法，参照 [差分数组技巧](https://labuladong.github.io/algo/di-yi-zhan-da78c/shou-ba-sh-48c1d/xiao-er-me-c31c8/) 的原理，有兴趣的读者可以自己尝试去实现。

**基于差分数组的思路，我们可以推导出一种更高效，更优雅的解法**。

我们首先把这些会议的时间区间进行投影：

[![img](https://labuladong.github.io/algo/images/%e5%ae%89%e6%8e%92%e4%bc%9a%e8%ae%ae%e5%ae%a4/1.jpeg)](https://labuladong.github.io/algo/images/安排会议室/1.jpeg)

红色的点代表每个会议的开始时间点，绿色的点代表每个会议的结束时间点。

现在假想有一条带着计数器的线，在时间线上从左至右进行扫描，每遇到红色的点，计数器 `count` 加一，每遇到绿色的点，计数器 `count` 减一：

[![img](https://labuladong.github.io/algo/images/%e5%ae%89%e6%8e%92%e4%bc%9a%e8%ae%ae%e5%ae%a4/2.jpeg)](https://labuladong.github.io/algo/images/安排会议室/2.jpeg)
...
[![img](https://labuladong.github.io/algo/images/%e5%ae%89%e6%8e%92%e4%bc%9a%e8%ae%ae%e5%ae%a4/3.jpeg)](https://labuladong.github.io/algo/images/安排会议室/3.jpeg)
...
这里使用的是 [双指针技巧](https://labuladong.github.io/algo/di-yi-zhan-da78c/shou-ba-sh-48c1d/shuang-zhi-fa4bd/)，根据 `i, j` 的相对位置模拟扫描线前进的过程。

至此，这道题就做完了。当然，这个题目也可以变形，比如给你若干会议，问你 `k` 个会议室够不够用，其实你套用本文的解法代码，也可以很轻松解决。

接下来可阅读：

- [区间问题系列合集](https://labuladong.github.io/algo/di-san-zha-24031/jing-dian--a94a0/yi-ge-fang-93124/)